...
public class VentanaMain extends JFrame {
	...
	public VentanaMain() {
		 ...
		 * Este botón:
		 * 
		 * 1. instancia la clase generada Lexico 2. analiza el contenido del JTextArea
		 * 3. muestra los resultados en el JList 4. crea el archivo que contiene la
		 * tabla de símbolos
		 */
						...
						for (Token token : lexer.getTokens()) {
							...
							if (TOKENS_TABLA_SIMBOLO.contains(token.getToken())
								...
								String fila = String.format("%-15s %-15s %-15s %-15s %-15s\n", nombre, token.getToken(),
										"---", token.getLexema(), longitud == -1 ? "---" : longitud);
								writer.write(fila);
							}
						}
